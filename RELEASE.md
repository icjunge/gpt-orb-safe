...
脚本不自动提交、推送或发布。成功后按它打印的命令提交和推送 `update-config.json`，再创建并推送 `v2.7.0` 标签。保护的签名作业会等待你在 GitHub Actions 审核；审核前检查构建和变更，批准后会签名、核验资产并正式发布。为便于单人维护，初始化允许发布者审核自己的发布；脚本不会执行审核或跳过审核。
...
1. 更新 `package.json` 与 `package-lock.json` 的版本（例如 `npm version 2.7.0 --no-git-tag-version`），同时将 `extension/manifest.json` 的 `version` 改为完全相同的 `X.Y.Z`（各段不带前导零且不超过 65535）。更新说明文档。
